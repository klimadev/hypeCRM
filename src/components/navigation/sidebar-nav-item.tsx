...
export function SidebarNavItem({ item, ativo, expanded, onNavigate, resumo }: Props) {
  ...
  const link = (
    <Link href={item.href} onClick={onNavigate} aria-label={`${item.label}: ${item.descricao}`} className={cn("group relative flex min-h-11 items-center gap-3 rounded-[var(--radius-control)] border border-transparent px-3 text-sm font-medium text-[var(--text-secondary)] transition-[transform,background-color,border-color,color,box-shadow] duration-150 ease-[var(--ease-productive)] hover:-translate-y-[1px] hover:border-[var(--border-subtle)] hover:bg-[color:rgba(255,255,255,0.04)] hover:text-[var(--text-primary)] active:scale-[0.98]", !expanded && "justify-center gap-0 px-0", ativo && "border-[color:rgba(139,92,246,0.24)] bg-[var(--brand-soft)] text-[var(--text-primary)] shadow-[0_12px_30px_-22px_rgba(139,92,246,0.65)]") }>
      <span className={cn("flex h-9 w-9 shrink-0 items-center justify-center rounded-[calc(var(--radius-control)-2px)] border bg-[color:rgba(255,255,255,0.03)] border-transparent", ativo && "border-[color:rgba(139,92,246,0.16)] bg-[color:rgba(139,92,246,0.14)]", !ativo && item.limpo && "border-[color:rgba(139,92,246,0.08)] bg-[color:rgba(139,92,246,0.04)]") }>
