      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <Button
            ...
            variant="ghost"
            size="sm"
            className="h-8 w-8 rounded-lg text-[var(--danger)] hover:bg-[color-mix(in_srgb,var(--danger)_12%,transparent)] hover:text-[var(--danger-strong)]"
            onClick={onAbrirRemocao}
            disabled={removendoNegocio}
          >
            <Trash2 className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg text-[var(--text-tertiary)] hover:bg-[var(--surface-soft)] hover:text-[var(--text-secondary)]" onClick={onFechar}>
            <X className="h-4 w-4" />
          </Button>
        </div>
      </div>
