...
function TagChip({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-flex min-h-8 items-center gap-1.5 rounded-full border border-[var(--border-subtle)] bg-[var(--surface-soft)] px-3 text-xs font-medium text-[var(--text-secondary)]">
      <Tags className="h-3.5 w-3.5 shrink-0 text-[var(--brand)]" />
      {children}
    </span>
  );
}
      ...
      <SheetContent className="z-[60] w-full max-w-[980px] overflow-hidden border-l border-[color:rgba(255,255,255,0.12)] bg-[color:#101010] shadow-[0_24px_90px_rgba(0,0,0,0.72)] backdrop-blur-xl" side="right">
        <SheetHeader className="sticky top-0 z-10 space-y-4 border-b border-[var(--border-subtle)] bg-[color:rgba(16,16,16,0.96)] p-5 backdrop-blur-xl">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              {telefoneNumerico ? (
                <Button asChild size="sm" className="rounded-xl bg-[var(--brand)] text-white hover:bg-[var(--brand-strong)]">
              ...
              <Button variant="outline" size="sm" className="rounded-xl" onClick={() => lead && onEditar(lead)} disabled={!lead}>
                <Pencil className="mr-2 h-4 w-4" />
                Editar
              </Button>
              <Button variant="outline" size="sm" className="rounded-xl" onClick={() => lead && onVincular(lead)} disabled={!lead}>
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5 pb-8">
          {carregando ? (
            ...
            <div className="rounded-xl border border-[color-mix(in_srgb,var(--danger)_35%,transparent)] bg-[color-mix(in_srgb,var(--danger)_8%,transparent)] p-4 text-sm text-[var(--danger)]">{erro}</div>
          ) : lead ? (
            <>
              ...
              <section className="rounded-xl border border-[color-mix(in_srgb,var(--brand)_22%,transparent)] bg-[color-mix(in_srgb,var(--brand)_6%,transparent)] p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button variant="outline" size="sm" className="rounded-xl" onClick={() => onVincular(lead)}>
              ...
              <section className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-elevated)] p-4">
                ...
                {lead.observacoes ? <p className="mt-4 rounded-xl bg-[var(--surface-soft)] p-3 text-sm leading-6 text-[var(--text-secondary)]">{lead.observacoes}</p> : null}
              </section>
              ...
              <section className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-elevated)] p-4">
                ...
                <div className="space-y-3">
                  {negocios.length > 0 ? negocios.map((negocio) => (
                    <div key={negocio.id} className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-soft)] p-3">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <Button asChild variant="ghost" size="sm" className="h-8 rounded-xl text-[var(--info)]">
                    ...
                    <div className="rounded-xl border border-dashed border-[var(--border-subtle)] bg-[var(--surface-soft)] p-4">
                      ...
                      <Button variant="outline" size="sm" className="mt-3 rounded-xl" onClick={() => onVincular(lead)}>
                        <Link2 className="mr-2 h-4 w-4" />
              ...
              <section className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-elevated)] p-4">
                ...
                <div className="space-y-2">
                  {informacoes?.interacoes.length ? informacoes.interacoes.map((interacao) => (
                    <div key={`${interacao.tipo}-${interacao.id}`} className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-soft)] p-3">
