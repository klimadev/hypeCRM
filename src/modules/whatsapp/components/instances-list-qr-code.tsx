...
export function InstancesListQrCode({ qrCode, phone, instanciaId, buscarQrCode }: QrCodeDisplayProps) {
  ...
  const corBarra = tempoRestante > 20 ? "bg-[var(--success)]" : tempoRestante > 10 ? "bg-[var(--warning)]" : "bg-[var(--danger)]";
  if (!qrAtual && !carregandoQr) return null;

  if (carregandoQr) {
    return (
      <div className="mt-4 flex flex-col items-center rounded-[var(--radius-card)] border border-dashed border-[var(--warning)] bg-[color-mix(in_srgb,var(--warning)_8%,transparent)] p-5">
