    ...
    <div className="grid gap-4 md:grid-cols-2">
      {camposOrdenados.map((campo) => {
        ...
        const larguraClasse = campo.largura === "full" ? "md:col-span-2" : campo.largura === "lg" ? "md:col-span-2" : "";
          ...
          return (
            <div key={campo.id} className={cn("rounded-[16px] border border-[var(--border-subtle)] bg-[var(--surface)] p-4", larguraClasse)}>
              <div className="mb-3">
                <label className="block text-sm font-medium text-[var(--text-primary)]">
                  {campo.label}
                  {campo.obrigatorio ? <span className="text-[var(--danger)]"> *</span> : null}
                </label>
                {campo.ajuda && (
                  <p className="mt-1 text-xs text-[var(--text-secondary)]">{campo.ajuda}</p>
                )}
              </div>
             ...
             {campo.tipo === "textarea" ? (
                ...
                <select
                  className="flex h-11 w-full rounded-[var(--radius-control)] border border-[var(--border-subtle)] bg-[var(--surface-elevated)] px-3 py-2 text-sm text-[var(--text-primary)] shadow-[var(--shadow-sm)]"
